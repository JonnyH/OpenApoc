...
void AEquipmentSheet::displayAmmo(sp<AEquipment> item, const AEquipmentType &itemType)
{
	...
	{
		form->findControlTyped<Label>("LABEL_10_C")->setText(tr("(Recharges)").value);
	}
}
                                    ...
                                    const AEquipmentType &itemType)
{
	...
	form->findControlTyped<Label>("LABEL_5_C")->setText(tr("Ammo types:").value);
	int ammoNum = 1;
...
{
	form->findControlTyped<Label>("LABEL_2_L")->setText(tr("Protection").value);
...
void AEquipmentSheet::displayAlien(sp<AEquipment> item, const AEquipmentType &itemType)
{
	form->findControlTyped<Label>("ITEM_NAME")
	    ->setText(itemType.bioStorage ? tr("Alien Organism").value : tr("Alien Artifact").value);
