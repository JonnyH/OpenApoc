    ...
    : Stage(), callbackYes(callbackYes), callbackNo(callbackNo), callbackCancel(callbackCancel)
{
	...
	switch (buttons)
	{
		...
		{
			auto bOk = form->createChild<TextButton>(tr("OK").value, ui().getFont("smallset"));
			bOk->Name = "BUTTON_OK";
		...
		{
			auto bYes = form->createChild<TextButton>(tr("Yes").value, ui().getFont("smallset"));
			...
			auto bNo = form->createChild<TextButton>(tr("No").value, ui().getFont("smallset"));
			bNo->Name = "BUTTON_NO";
		...
		case ButtonOptions::YesNoCancel:
		{
			...
			auto bCan = form->createChild<TextButton>(tr("Cancel").value, ui().getFont("smallset"));
