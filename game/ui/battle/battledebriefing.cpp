    ...
    : Stage(), menuform(ui().getForm("battle/debriefing")), state(state)
{
	...
	menuform->findControlTyped<Label>("TEXT_SCORE_COMBAT_RATING")
	    ->setText(OpenApoc::format2("{0}", state->current_battle->score.combatRating));
	menuform->findControlTyped<Label>("TEXT_SCORE_CASUALTY_PENALTY")
	    ->setText(OpenApoc::format2("{0}", state->current_battle->score.casualtyPenalty));
	menuform->findControlTyped<Label>("TEXT_SCORE_LEADERSHIP_BONUS")
	    ->setText(OpenApoc::format2("{0}", state->current_battle->score.getLeadershipBonus()));
	menuform->findControlTyped<Label>("TEXT_SCORE_LIVE_ALIENS_CAPTURED")
	    ->setText(OpenApoc::format2("{0}", state->current_battle->score.liveAlienCaptured));
	menuform->findControlTyped<Label>("TEXT_SCORE_EQUIPMENT_CAPTURED")
	    ->setText(OpenApoc::format2("{0}", state->current_battle->score.equipmentCaptured));
	menuform->findControlTyped<Label>("TEXT_SCORE_EQUIPMENT_LOST")
	    ->setText(OpenApoc::format2("{0}", state->current_battle->score.equipmentLost));
	menuform->findControlTyped<Label>("TEXT_SCORE_TOTAL")
	    ->setText(OpenApoc::format2("{0}", state->current_battle->score.getTotal()));
	menuform->findControlTyped<Label>("TEXT_MISSION_PERFORMANCE")
	    ->setText(OpenApoc::format2("{0}", state->current_battle->score.getText()));
