...
Projectile::Projectile(Type type, StateRef<Vehicle> firer, StateRef<Vehicle> target,
    ...
    : type(type), position(position), velocity(velocity), turnRate(turnRate), age(0),
      ...
      projectile_sprites(std::move((projectile_sprites))), sprite_distance(1.0f / TILE_Y_CITY),
      voxelMapLof(turnRate > 0 ? voxelMap : nullptr), voxelMapLos(voxelMap), manualFire(manualFire),
      impactSfx(std::move((impactSfx))), doodadType(doodadType), velocityScale(VELOCITY_SCALE_CITY),
      stunTicks(stunTicks), splitIntoTypesCity(std::move((splitIntoTypes)))
{
...
Projectile::Projectile(Type type, StateRef<BattleUnit> firer, StateRef<BattleUnit> target,
    ...
    : type(type), position(position), velocity(velocity), turnRate(turnRate), age(0),
      ...
      spritePositions({position}), tail_length(tail_length),
      projectile_sprites(std::move((projectile_sprites))), sprite_distance(1.0f / TILE_Y_BATTLE),
      voxelMapLof(turnRate > 0 ? voxelMap : nullptr), voxelMapLos(voxelMap), manualFire(manualFire),
      impactSfx(std::move((impactSfx))), doodadType(doodadType), damageType(damageType),
      velocityScale(VELOCITY_SCALE_BATTLE), stunTicks(stunTicks),
      splitIntoTypesBattle(std::move((splitIntoTypes)))
{
