                     ...
                     std::map<OpenApoc::StateRef<OpenApoc::AgentType>, int> &target)
{
	if (crew.alien_egg > 0)
	{
		target[{&state, "MULTIWORM_EGG"}] = crew.alien_egg;
	}
	if (crew.anthropod > 0)
	{
		target[{&state, "ANTHROPOD"}] = crew.anthropod;
	}
	if (crew.brainsucker > 0)
	{
		target[{&state, "BRAINSUCKER"}] = crew.brainsucker;
	}
	if (crew.crysalis > 0)
	{
		target[{&state, "CHRYSALIS"}] = crew.crysalis;
	}
	if (crew.hyperworm > 0)
	{
		target[{&state, "HYPERWORM"}] = crew.hyperworm;
	}
	if (crew.megaspawn > 0)
	{
		target[{&state, "MEGASPAWN"}] = crew.megaspawn;
	}
	if (crew.micronoid > 0)
	{
		target[{&state, "MICRONOID_AGGREGATE"}] = crew.micronoid;
	}
	if (crew.multiworm > 0)
	{
		target[{&state, "MULTIWORM"}] = crew.multiworm;
	}
	if (crew.popper > 0)
	{
		target[{&state, "POPPER"}] = crew.popper;
	}
	if (crew.psimorph > 0)
	{
		target[{&state, "PSIMORPH"}] = crew.psimorph;
	}
	if (crew.queenspawn > 0)
	{
		target[{&state, "QUEENSPAWN"}] = crew.queenspawn;
	}
	if (crew.skeletoid > 0)
	{
		target[{&state, "SKELETOID"}] = crew.skeletoid;
	}
	if (crew.spitter > 0)
	{
		target[{&state, "SPITTER"}] = crew.spitter;
	}
}
