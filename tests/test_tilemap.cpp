...
class FakeSceneryTileObject : public TileObject
{
  public:
	...
	void draw(Renderer &, TileTransform &, Vec2<float>, TileViewMode, bool, int, bool,
	          bool) override
	{
		LogError2("DRAW CALLED ON FAKE SCENERY??");
		exit(EXIT_FAILURE);
	}
};
                           ...
                           sp<TileObject> expected_collision)
{
	...
	{
		LogError2("Line between {{{:f},{:f},{:f}}} and {{{:f},{:f},{:f}}} collided with {}, expected {}",
...
int main(int argc, char **argv)
{
	...
	for (auto &object : objects)
	{
		...
		{
			LogError2("Object {} moved from {{{:f},{:f},{:f}}} to {{{:f},{:f},{:f}}}", object.second->getName(),
